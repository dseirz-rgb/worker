...
function TrainOfThoughtComponent(props: TrainOfThoughtComponentProps) {
  ...
  // 转换为统一格式
  const trainOfThoughtObjects: TrainOfThoughtObject[] = 
    typeof props.trainOfThought[0] === 'string'
      ? (props.trainOfThought as string[]).map((data) => ({ type: 'text', data }))
      : (props.trainOfThought as TrainOfThoughtObject[]);

  if (!trainOfThoughtObjects.length) return null;

  return (
    <div className={`${!collapsed ? 'border rounded-lg p-3 mb-3' : ''}`} key={props.keyId}>
      {!props.completed && (
        ...
        {!collapsed && (
          ...
          >
            <div className="space-y-2 mt-2">
              {trainOfThoughtObjects.map((entry, index) => (
                <TrainOfThought
                  key={`${props.keyId}-${index}`}
                  message={entry.data}
                  primary={index === trainOfThoughtObjects.length - 1 && props.lastMessage && !props.completed}
