...
// Echo 扩展: 划词翻译处理
// 获取选中文本并发送到前端进行翻译
pub fn handle_selection_translate<R: Runtime>(app: &AppHandle<R>) {
    println!("🌐 Selection translate triggered!");

    // 获取选中文本
    let selected_text = match get_selected_text_directly() {
        Ok(text) if !text.is_empty() && text.trim().len() > 0 => {
            println!("📋 Got selected text for translation: '{}' (length: {})", text, text.len());
            text
        }
        Ok(_) => {
            println!("⚠️ Selected text is empty");
            return;
        }
        Err(e) => {
            eprintln!("❌ Failed to get selected text: {}", e);
    ...
    if let Some(window) = app.get_webview_window("main") {
        match window.emit("selection-translate-triggered", &event) {
            Ok(_) => println!("📡 Successfully emitted selection translate event"),
            Err(e) => eprintln!("❌ Failed to emit selection translate event: {}", e),
        }
    }

    // 也发送全局事件
    match app.emit("selection-translate-triggered", &event) {
        Ok(_) => println!("📡 Successfully emitted global selection translate event"),
        Err(e) => eprintln!("❌ Failed to emit global selection translate event: {}", e),
    }
}
