            ...
            if let Some(command) = shortcuts_map.get(&shortcut_str.to_lowercase()) {
                ...
                match command.as_str() {
                    ...
                    "screenshot-translate" => {
                        println!("📸 Triggering screenshot translate via shortcut: {}", shortcut_str);
                        // 发送事件到前端处理截图翻译
                        if let Some(window) = app.get_webview_window("main") {
                            let _ = window.emit("screenshot-translate-triggered", ());
                        }
                        return;
                    },
                    "selection-translate" => {
                        println!("📝 Triggering selection translate via shortcut: {}", shortcut_str);
                        // 获取选中文本并发送到前端翻译
                        crate::desktop::handle_selection_translate(app);
                        return;
                    },
                    _ => {
            ...
            for (registered_shortcut, command) in shortcuts_map.iter() {
                ...
                if shortcuts_match(&shortcut_str, registered_shortcut) {
                    ...
                    match command.as_str() {
                        ...
                        // Echo 扩展: 翻译快捷键
                        "screenshot-translate" => {
                            println!("📸 Triggering screenshot translate via matched shortcut: {} -> {}", shortcut_str, registered_shortcut);
                            if let Some(window) = app.get_webview_window("main") {
                                let _ = window.emit("screenshot-translate-triggered", ());
                            }
                            return;
                        ...
                        "selection-translate" => {
                            println!("📝 Triggering selection translate via matched shortcut: {} -> {}", shortcut_str, registered_shortcut);
                            crate::desktop::handle_selection_translate(app);
                            return;
                        },
                        _ => {
